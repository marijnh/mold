...
function dispatchCustom(name, arg, output) {
  if (!custom.hasOwnProperty(name))
    throw new Error("Unrecognised template command: '" + name + "'.");
  output.push(custom[name](arg, output));
}

function evalTemplate(code, ctx) {
  if (!ctx) ctx = {};
  ctx._dispatchCustom = dispatchCustom;
  ctx._forEachIn = forEachIn;
  ctx._escapeHTML = escapeHTML;
  var prelude = "";
  for (var prop in ctx) if (ctx.hasOwnProperty(prop))
    prelude += "var " + prop + "=__ctx." + prop + ";\n";
  return new Function("__ctx", "$arg", "__output", prelude + code).bind(null, ctx);
}
